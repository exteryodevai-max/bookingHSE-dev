...
export default function Dashboard() {
  ...
  const getStatusLabel = useCallback((status: string) => {
    return statusConfig.labels[status] || status;
  }, [statusConfig.labels]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">
            Benvenuto, {user?.user_type === 'client'
                    ? user.client_profiles?.company_name
                    : user?.provider_profiles?.business_name}
          </h1>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <div className="mt-2 text-sm text-gray-600">
                            {user?.user_type === 'client' 
                              ? `Fornitore: ${booking.provider?.provider_profiles?.business_name || 'N/A'}`
                              : `Cliente: ${booking.client?.client_profiles?.company_name || 'N/A'}`
                            }
                          </div>
                        </div>
