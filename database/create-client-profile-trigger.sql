-- Trigger per creare automaticamente il profilo cliente quando viene creato un utente di tipo "client"
...
-- Trigger per aggiornare automaticamente il company_name nel profilo cliente
-- quando viene aggiornato il company_name nella tabella users
...
COMMENT ON FUNCTION create_client_profile_on_user_insert() IS 'Crea automaticamente un profilo cliente quando viene inserito un utente di tipo client';
COMMENT ON FUNCTION update_client_profile_company_name() IS 'Aggiorna automaticamente il company_name nel profilo cliente quando viene aggiornato nella tabella users';
COMMENT ON TRIGGER create_client_profile_trigger ON users IS 'Trigger per la creazione automatica del profilo cliente';
COMMENT ON TRIGGER update_client_profile_company_name_trigger ON users IS 'Trigger per l\'aggiornamento automatico del company_name nel profilo cliente';